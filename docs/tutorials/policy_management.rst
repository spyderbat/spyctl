...
One example of adding a custom |action| is to receive a Slack notification via webhook when a
|policy| is violated.
...
.. note:: 
    Adding custom |responses| is completely optional. When a |policy| is enforcing,
...
to generate updated |fprints| which may contain activity that deviates from the |policy|. The way to detect this
...
For example, to apply the |policy| we created above via the |api|, we issue the following command:
...
.. warning:: 
    ...
    and take custom response actions you have defined.
