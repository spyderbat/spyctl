...
from spyctl.policies import POL_TYPE_CONT, POL_TYPE_SVC
...
def parse_args():
    ...
    parser.add_argument('--deployment', help='name of deployment to use from user configuration file', dest="selected_deployment")
...
def make_configure(subs: _SubParsersAction[ArgumentParser]):
    ...
    configure_subs = configure.add_subparsers(title="configure commands", dest="config_cmd")
...
def make_configure_add(configure_subs: _SubParsersAction[ArgumentParser]):
    ...
    configure_add = configure_subs.add_parser("add", description=desc, formatter_class=fmt)
    configure_add.add_argument('deployment_name', help="the name of the deployment to add")
    configure_add.add_argument('-k', '--api_key', metavar="KEY", required=True)
    configure_add.add_argument('-u', '--api_url', help=f"Default: {DEFAULT_API_URL}", default=DEFAULT_API_URL, metavar="URL")
    configure_add.add_argument('-o', '--org', metavar="NAME_OR_UID", required=True)
    configure_add.add_argument('-D', '--set-default', action="store_true", help="set config as default")
...
def make_configure_update(configure_subs: _SubParsersAction[ArgumentParser]):
    ...
    configure_update = configure_subs.add_parser("update", description=desc, formatter_class=fmt)
    configure_update.add_argument('deployment_name', help="the name of the deployment to update")
    ...
    configure_update.add_argument('-D', '--set-default', action="store_true", help="set deployment as default")


def make_configure_setdefault(configure_subs: _SubParsersAction[ArgumentParser]):
    desc = "set a Spyderbat API deployment as default in spyctl user configuration"
    configure_default = configure_subs.add_parser("default", description=desc, formatter_class=fmt)
    configure_default.add_argument('deployment_name', help="the name of the deployment to set as default")
...
def make_configure_delete(configure_subs: _SubParsersAction[ArgumentParser]):
    ...
    configure_delete = configure_subs.add_parser("delete", description=desc, formatter_class=fmt)
    configure_delete.add_argument('deployment_name', help="the name of the deployment to set as default")


def make_configure_show(configure_subs: _SubParsersAction[ArgumentParser]):
    desc = "show the user configuration file"
    configure_show = configure_subs.add_parser("show", description=desc, formatter_class=fmt)
...
def make_create_spyderbat_policy(create_subs: _SubParsersAction[ArgumentParser]):
    names = ['spyderbat-policy', 'spyderbat-policies', 'spy-pol', 'spyderbat', 'spol', 'sp']
    create_spyderbat_policy = create_subs.add_parser(**name_and_aliases(names), formatter_class=fmt)
    create_spyderbat_policy.add_argument('-F', "--fingerprints", help="fingerprint(s) to build a policy from", nargs='?', const='-')
    create_spyderbat_policy.add_argument('type', choices=[POL_TYPE_CONT, POL_TYPE_SVC], help="the type of policy to create")
...
def make_upload(subs: _SubParsersAction[ArgumentParser]):
    desc = "upload objects to Spyderbat"
